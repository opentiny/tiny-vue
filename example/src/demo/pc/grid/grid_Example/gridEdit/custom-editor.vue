<template>
  <div>
    <tiny-grid :data="tableData" :edit-config="{ trigger: 'click', mode: 'cell', showStatus: true }">
      ...
      <tiny-grid-column field="name" title="默认自定义列编辑" :editor="{ component: CustomInputDefault }"></tiny-grid-column>
      <tiny-grid-column field="contact" title="自定义列编辑" :editor="{ component: CustomInput }"></tiny-grid-column>
...
const CustomInput = {
  inheritAttrs: false,
  template: '<input class="grid-editor-item" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" />',
  model: {
    prop: 'modelValue',
    event: 'update:modelValue'
  },

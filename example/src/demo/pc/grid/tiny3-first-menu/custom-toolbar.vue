<template>
  <div>
    <tiny-grid :fetch-data="fetchData" seq-serial ref="basicGrid" :edit-config="{ trigger: 'click', mode: 'cell', showStatus: true }" :pager="pagerConfig">
      <template #toolbar>
        <tiny-grid-toolbar>
          <template #buttons>
      ...
      <tiny-grid-column field="area" title="区域" sortable :editor="{ component: 'select', options: options }"></tiny-grid-column>
  ...
  methods: {
    getData({ page }) {
      ...
      return new Promise((resolve) => {
        resolve({
          result: this.tableData.slice(offset, offset + pageSize).filter((row) => row.name.indexOf(this.text) !== -1),

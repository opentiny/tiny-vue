<template>
  <div>
    ...
    <tiny-button @click="downloadMore">批量下载</tiny-button>
    <tiny-grid :data="tableData" ref="fileListGrid" height="370">
      <tiny-grid-column type="selection" width="4%"></tiny-grid-column>
      <tiny-grid-column field="docId" title="文档ID" width="20%"></tiny-grid-column>
      ...
      <tiny-grid-column field="serverName" title="存储站点" width="13%"></tiny-grid-column>
    </tiny-grid>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      edm: {
        ...
        download: {
          packageToken: () =>
            new Promise((resolve) => {
              // resolve 集成EDM sdk 或获取的上传token ，示例格式如下（示例数据不可用）：
              //  resolve({
              //   config:{fileWhiteList:".doc,.docx,.xls"},
              //   edmToken:"security:2B263FA5CF0940A77008C809:041F85244E7BCB01781462D5F0BD68934D51DF3F2192ACB36640A7DC8AFD8378B238B0881B5F2FDF",
              //   traceId:"0b1246cf-a1c5-418c-bd3b-543039407951"
  ...
  methods: {
    ...
    downloadMore() {
      const files = this.$refs.fileListGrid.getSelectRecords()
      ...
      this.$refs.upload.downloadFile(files)
    }
  }
}
</script>

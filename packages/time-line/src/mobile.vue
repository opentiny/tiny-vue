<template>
  <div :class="['tiny-mobile-timeline']">
    <div
      ...
      :style="{
        height:
          index === state.nodes.length - 1 ? '' : space ? space + 'px' : '88px'
      }"
      ...
      <ul class="tiny-mobile-timeline__list">
        <slot name="left" :slot-scope="node">
          <li class="tiny-mobile-timeline__date-time">
            <span class="tiny-mobile-timeline__date">{{
              getDate(node[timeField]).date
            }}</span>
            <span class="tiny-mobile-timeline__time">{{
        ...
        <slot name="right" :slot-scope="node">
          <li class="tiny-mobile-timeline__content">
            <div>
              <span class="tiny-mobile-timeline__title">{{
                node[nameField]
              }}</span>
              <span class="tiny-mobile-timeline__cycle">{{ node.cycle }}</span>
            </div>
            <div>
              <span class="tiny-mobile-timeline__user">{{
                node.personInfo
              }}</span>

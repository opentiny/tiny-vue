    ...
    >
      ...
      >
        <slot name="index" :value="state.index + 1">
          <span class="tiny-mobile-image-viewer__index--curren">{{
            state.index + 1
          }}</span>
          <span>/</span>
          <span class="tiny-mobile-image-viewer__index--total">{{
            urlList.length
          }}</span>
        </slot>
      </div>
      ...
      >
        <div class="tiny-mobile-image-viewer__actions-inner">
          <slot name="tool">
            ...
            <icon-repeat
              class="tiny-svg-size"
              @click.stop="handleActions('anticlocelise')"
            />
            <icon-refres
              class="tiny-svg-size"
              @click.stop="handleActions('clocelise')"
            />
            <icon-del
              class="tiny-svg-size"
              v-if="deleteButton"
      ...
      <div class="tiny-mobile-image-viewer__canvas">
        ...
        >
          <div
            ...
            :style="
              Object.assign(
                { width: `${state.imageItemWidth}px` },
                i === state.index ? state.imgStyle : ''
              )
            "
          >
            <div

...
const Methods = {
  ...
  handleTableData(force) {
    let { scrollYLoad, scrollYStore, treeConfig, treeExpandeds } = this
    let { renderSize, startIndex } = scrollYStore
    let afterFullData = force ? this.updateAfterFullData() : this.afterFullData

    this.tableData = sliceFullData({ afterFullData, renderSize, scrollYLoad, startIndex, treeConfig, treeExpandeds })
  ...
  updateAfterFullData() {
    ...
    let doColumnSort = () => {
      let sorted = sortColumn.sortMethod ? tableData.sort(sortColumn.sortMethod) : sortBy(tableData, sortColumn.property)
  ...
  triggerHeaderCellClickEvent(event, params) {
    ...
    if (trigger === 'cell' && !(isResizable || isSort || isFilter)) {
      let nextOrder = !column.order ? 'asc' : column.order === 'asc' ? 'desc' : null
  ...
  updateStatus(scope, cellValue, renderOpts) {
    ...
    return this.$nextTick().then(() => {
      ...
      if (renderOpts.isValidAlways && renderOpts.isValidAlways) {

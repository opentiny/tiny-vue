    ...
    export default function actionSheet(props) {
      const {menus,modelValue,visible,ellipsis,contentPosition,contentStyle,height,valueField,textField,contentClass} = props
      const defaultProps = {
        ...$props,
        ...props,
        menus,modelValue,visible,ellipsis,contentPosition,contentStyle,height,valueField,textField,contentClass
      }
...
<div  style={state.sheetMaskStyle} style={{display:!contentPosition?'block':'none'}} className="tiny-mobile-action-sheet__mask"></div>

    
<div  style={[state.sheetContentStyle]} ref={scrollMenu} v-ref="scrollMenu" className={vc([
...
<div  className={vc(['tiny-mobile-action-sheet__menu', ellipsis ? 'is-ellipsis' : ''])}>        

  {
    menus.map((item, index)=>(
      <div  style={state.contentStyle}  key={index} onClick={selectOption(item, index)} className={vc([

    ...
    <div class="tiny-fall-menu__wrap">
      <div class="tiny-fall-menu__nav">
        <div class="tiny-fall-menu__subnav">
          ...
          >
            <ul>
              <li
                ...
                :class="{
                  'fall-hide':
                    index >= state.pagerData.index[state.pager - 1] &&
                    state.pager !== state.pagerData.size
                }"
              >
                <slot name="level1" :slot-scope="level1">
                  <a
                    :href="level1.url"
                    :class="{ now: index === state.activeNode }"
                    >{{ level1.title }}
                  </a>
                </slot>
              </li>
          ...
          <div
            v-show="state.pager !== state.pagerData.size"

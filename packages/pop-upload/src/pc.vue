<template>
  <div class="tiny-popupload">
    <tiny-button :size="size" @click="showDialog" :disabled="state.disabled">{{
    ...
    >
      <div class="tiny-popupload__dialog">
        <div class="tiny-popupload__dialog-header">
          ...
          >
            <div class="tiny-popupload__dialog-tips">
              <p
                v-for="(item, index) in state.tipsTitle.filter(
              ...
              >
                {{
                  item.count != undefined
                    ? state.limitCountTips + ':' + item.count
                    : ''
                }}
                {{
                  item.type != undefined
                    ? state.limitTypeTips + ':' + item.type
                    : ''
                }}
                {{
                  item.size != undefined
                    ? state.limitSizeTips + ':' + item.size
                    : ''
                }}
              </p>
            </div>
          ...
          <tiny-alert
            type="error"
            size="large"
            v-if="state.errorTips.length > 0"
            @close="closeErrorTips"
          >
            <div class="tiny-popupload__dialog-tips">
              <p v-for="(item, index) in state.errorTips" :key="index">
                {{
                  item.size != undefined
                    ? item.size + ':' + state.errorSizeTips
                    : ''
                }}
                {{
                  item.type != undefined
                    ? item.type + ':' + state.errorTypeTips
                    : ''
                }}
          ...
          <tiny-alert
            type="success"
            size="large"
            v-if="state.successTips.length > 0"
            @close="closeSuccessTips"
          >
            <div class="tiny-popupload__dialog-tips">
              <p v-for="(item, index) in state.successTips" :key="index">
            ...
            <div class="tiny-popupload__dialog-table-body">
              <ul class="tiny-popupload__dialog-table-list">
                <li
                  ...
                  v-for="(item, index) in state.uploadList"
                  :key="index"
                >
                  <p class="body-col col1">{{ item.name }}</p>
                  <p class="body-col col2">
                    {{ Math.floor(Number(item.size) / 1024) }}KB
                  </p>
        ...
        <div class="tiny-popupload__dialog-footer">
          <tiny-button
            type="primary"
            :disabled="state.uploadList.length == 0"
            @click="uploadSubmit"
            >{{ state.submitButtonText }}</tiny-button
          >
...
import {
  renderless,
  api
} from '@opentiny/vue-renderless/pop-upload/vue'

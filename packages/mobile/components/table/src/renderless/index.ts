...
export const computedCheckableData =
  ({ props }) =>
  () =>
    props.data.filter((item) => !item.disabled)

export const computedSelectedKeys =
  ({ props, state }) =>
  () => {
    const keys = []
...
export const computedSelectCls =
  ({ state }) =>
  () => {
...
export const watchDefaultChecked =
  ({ props, state }) =>
  (value, oldvalue) => {

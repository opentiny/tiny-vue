  ...
  >
    <template v-if="type !== 'textarea'">
      ...
      <span
        ...
        class="
          ...
          top-1/2
          -translate-y-1/2
          text-center text-color-text-placeholder
          flex
          items-center
        "
      ...
      <div ref="panel" v-if="slots.panel">
    ...
    <textarea
      ...
      class="
        block
        border-0
        sm:border-solid sm:border-color-border
        sm:hover:border-color-border-hover
        sm:focus:border-color-brand-focus
        sm:disabled:border-color-border
        outline-0
        rounded
        placeholder:text-color-text-placeholder placeholder:text-xs
        text-xs text-color-text-primary
        bg-white
        disabled:cursor-not-allowed
        disabled:text-color-text-disabled
        disabled:bg-color-bg-2
        leading-normal
      "
      :class="[readonly ? 'sm:border-0 px-0 py-0' : 'sm:border px-3 py-2']"
      @compositionstart="handleCompositionStart"
      @compositionupdate="handleCompositionUpdate"
      @compositionend="handleCompositionEnd"
      @input="handleInput"

<template>
  <div role="carousel-item" v-show="state.ready" :class="
    m([
  ...
  " :style="state.getTransform" @click="handleItemClick">
    <div v-if="state.carouselParent.type === 'card'" v-show="!state.active" class="absolute h-full top-0 left-0 w-full opacity-[24] duration-200 group-hover:opacity-[12]" :class="{ 'opacity-[12]': state.inStage && state.carouselParent.type === 'card' && state.hover }"></div>
    <slot></slot>
    <div v-if="state.hasTitle" class="absolute bottom-0 h-6 w-full text-left text-color-text-inverse leading-6 bg-color-text-disabled">

<template>
  <div data-tag="tiny-mobile-calendar-bar" class="select-none">
    ...
    <div class="w-full h-max">
      <div class="grid grid-cols-7">
        <div v-for="weekDay in state.weekDays" :key="weekDay" :style="state.minSize2" class="text-xs">
          <calendar-item :text="t(`ui.calendarBar.week.${weekDay}`)" :day="`${weekDay}`" :config="config"></calendar-item>
        </div>
      </div>
      <div class="w-full h-max" @touchstart="handleDraggerTouchstart" @touchend="handleDraggerTouchend" @touchmove.prevent="throttledHandleDraggerTouchmove">
        <div class="grid grid-cols-7" ref="calendarPanel">
          <div v-for="calendarDay in state.filteredCalendarDays" :key="calendarDay.dateStr" :style="state.minSize3" :class="
            m(
              'text-sm',
          ...
          ">
            <calendar-item :text="`${calendarDay.dateArr[2]}`" :day="calendarDay" :config="config" @clickday="handleClickDay"></calendar-item>
          </div>
        </div>
        <div class="w-full h-4 flex justify-center items-center">
          <div class="w-6 h-full flex justify-center items-center" @click="handleDraggerClick" @mousedown="handleDraggerMousedown">
            <div :class="[
    ...
    <tiny-cascader-select v-model="state.cascaderCurrent" cycle-roll :options="state.cascaderOptions" :visible="state.cascaderVisible" @update:modelValue="handleCascaderChange" @update:visible="setCascaderVisible" v-bind="config.cascaderOp"></tiny-cascader-select>
  </div>
</template>

import { isArray, isBoolean } from '../../utils/static'
import { getFilters, emitEvent } from '../../utils/utils'
...
export default {
  ...
  confirmFilterEvent() {
    ...
    // 3、抛出filter-change事件（在服务端筛选时grid会注册filter-change事件处理，然后grid会再次抛出filter-change）

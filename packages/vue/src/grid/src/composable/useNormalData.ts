...
export const useNormalData = ({ props, visibleColumn }) => {
  ...
  // 监听 rawData 变化（数据源变化）
  hooks.watch(rawData, (_data) => {
    if (props.editConfig && Array.isArray(_data) && _data.length > 0 && visibleColumn.value?.length > 0) {
  ...
  // 监听 visibleColumn 变化（列配置变化，如个性化设置）
  hooks.watch(
    () => visibleColumn.value,
    (_visibleColumn) => {
      if (props.editConfig && _visibleColumn?.length > 0) {
        rawData.value.forEach((row) => $table.defineField(row))
        const insertList = $table.editStore.insertList
        insertList.forEach((row) => $table.defineField(row))
      }
    }
  )

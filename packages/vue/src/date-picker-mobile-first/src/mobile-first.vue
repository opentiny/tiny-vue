<template>
  <div data-tag="tiny-date-picker-mobile">
    <tiny-action-sheet :title="title" custom-class="h-full max-h-[100%] rounded-none" :visible="visible" @update:visible="$emit('update:visible', $event)">
      <!-- box -->
      <div data-tag="tiny-date-picker-inner" :class="['flex flex-col flex-auto h-full']">
        <!-- header -->
        <div data-tag="date-picker-header" class="flex leading-6 py-2.5 px-4 text-sm items-center text-center border-b border-color-bg-4 shadow-sm">
          ...
          <div v-for="yearMonth in state.months" :key="yearMonth" class="mt-4">
            ...
            <div class="text-center leading-9 text-xs font-medium">
              <div v-for="(item, week) in getWeeksByMonth(yearMonth)" :key="week" :class="['flex mt-2']">
                <div v-for="(item, index) in 7" :key="index" :class="['flex-1 h-9 cursor-pointer', { 'text-color-text-placeholder': index === 0 || index === 6 }]" @click.stop="selectOption({ yearMonth, index: index + week * 7 })">
        ...
        <!-- footer -->
        <div data-tag="date-picker-footer" class="flex flex-col items-center justify-center p-4 border-t border-color-bg-4 shadow-sm">
          <div data-tag="rang-box" class="w-full" v-if="['datetimerange', 'daterange'].includes(type) && state.date.length">
            <div class="w-full flex mb-4 leading-6 text-base text-center">
              ...
              <div class="flex-auto min-w-[1rem]">
                <div v-if="['datetimerange'].includes(type)" class="h-9 h-9 border-l-0.5 inline-block rotate-45 relative top-6 border-color-bg-2"></div>
              </div>
          ...
          <div class="w-full text-center">
            <tiny-button tiny_mode="mobile-first" button-class="w-full" :disabled="state.btnDisabled" type="primary" size="medium" :reset-time="0" @click="confirm">
              {{ t('ui.button.confirm') }}
            </tiny-button>
          </div>
        </div>
    ...
    <tiny-time-picker-mobile v-model="state.time" title="时间选择" :visible="state.timeVisible" @update:visible="state.timeVisible = $event" @confirm="timeConfirm">

...
export default function () {
  function handleClearEvent(params, event) {
    ...
    if (
      getTarget(event, bodyEl, 'el-autocomplete-suggestion').flag ||
      getTarget(event, bodyEl, 'el-select-dropdown').flag ||
      getTarget(event, bodyEl, 'el-cascader__dropdown').flag ||
      getTarget(event, bodyEl, 'el-cascader-menus').flag ||
      getTarget(event, bodyEl, 'el-picker-panel').flag
    ) {
      return false
    }
  }

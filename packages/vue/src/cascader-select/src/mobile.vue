<template>
  <tiny-action-sheet data-tag="tiny-mobile-cascader-select" content-class="rounded-t-lg bg-white" :visible="state.actionSheetVisible" @update:visible="state.actionSheetVisible = $event">
    <div class="w-full h-max rounded-t-lg bg-white select-none overflow-hidden" @click.stop>
      <div ref="selectPanel" :class="[
        'w-full h-max grid',
        { 'grid-cols-1': state.selectorCount === 1 },
        { 'grid-cols-2': state.selectorCount === 2 },
        { 'grid-cols-3': state.selectorCount === 3 }
      ]">
        <div v-for="(v, i) in state.selectorCount" :key="`s${i}`" class="h-40" style="transform: translate3d(0, 0, 0)" @touchstart.prevent="handleTouchstart" @touchend.prevent="($event) => handleTouchend($event, i)" @touchmove.prevent="($event) => throttledHandleTouchmove($event, i)">
          <div v-for="(option, j) in state[`selector${i}`].visibleOptions" data-tag="tiny-mobile-cs-option" :date-status="statusOpt(option)" :data-option="option[valueField]" :key="`s${i}o${j}`" :class="
            m(
      ...
      <div class="w-full h-11 flex justify-center items-center">
        <tiny-button type="primary" size="large" class="bg-color-icon-focus h-10 flex justify-center items-center" :disabled="state.confirmDisabled" @click="confirm">{{ t('ui.button.confirm') }}</tiny-button>
      </div>
    </div>
  </tiny-action-sheet>
</template>

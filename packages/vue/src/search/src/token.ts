export const classes = {
  ...
  'pc-search-line':
    'w-full border border-solid rounded-full lg:rounded bg-color-bg-1 transition duration-300 ease-in-out border-separate hidden sm:inline-table',
  ...
  'pc-search-input-big':
    'pl-3 text-sm h-[calc(theme(spacing.8)-2*theme(borderWidth.DEFAULT))] leading-[calc(theme(spacing.8)-2*theme(borderWidth.DEFAULT))]',
  ...
  'search-selector':
    'absolute top-full left-0 overflow-hidden min-w-[theme(spacing.18)] border border-solid border-color-border rounded text-xs shadow bg-white text-color-text-primary mt-0.5 py-1 hidden sm:block',

<template>
  <div data-tag="tiny-drawer" class="text-sm" v-show="visible">
    <!-- mask -->
    <div ref="mask" v-if="mask" :class="
      m('fixed z-50 inset-0 w-full h-full bg-black opacity-0 transition-opacity ease-linear duration-200', {
        'opacity-30': state.toggle
      })
    " @click="maskClosable && close()"></div>
    <!-- main -->
    <div data-tag="tiny-drawer-main" ref="drawerBox" :class="
      m(
    ...
    " :style="{ width: ['left', 'right'].includes(placement) ? state.computedWidth : null }">
      <div data-tag="drawer-drag-bar" ref="dragBar" v-if="dragable" :class="['h-full absolute top-0 w-2 cursor-e-resize', placement === 'left' ? '-right-1' : '-left-1']"></div>
      <div :class="['flex-auto flex-col flex max-h-full']">
        <!-- header -->
        <div data-tag="drawer-header" ref="header" v-if="showHeader" class="flex leading-6 p-4 text-base items-center">
          <slot name="header">
        ...
        <!-- body -->
        <div data-tag="drawer-body" ref="body" class="flex-auto flex flex-col overflow-auto">
          <slot></slot>
        </div>
        <!-- footer -->

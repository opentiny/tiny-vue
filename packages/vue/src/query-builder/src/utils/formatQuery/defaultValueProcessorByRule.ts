...
export const defaultValueProcessorByRule: ValueProcessorByRule = (
  ...
  if (operatorLowerCase === 'null' || operatorLowerCase === 'notnull') {
    ...
    if (valueAsArray.length > 0) {
      return `(${valueAsArray
        .map((v) =>
          valueIsField || shouldRenderAsNumber(v, parseNumbers) ? `${trimIfString(v)}` : `'${escapeSingleQuotes(v)}'`
        )
        .join(', ')})`
    }
    return ''
    ...
    if (valueAsArray.length >= 2 && isValidValue(valueAsArray[0]) && isValidValue(valueAsArray[1])) {
      ...
      return valueIsField || (shouldRenderAsNumber(first, parseNumbers) && shouldRenderAsNumber(second, parseNumbers))

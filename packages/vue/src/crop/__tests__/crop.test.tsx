...
describe('PC Mode', () => {
  ...
  test.todo(
    'view-mode 定义 cropper 的视图模式;默认为 0;0：没有限制，3 可以移动到 2 外。1：3 只能在 2 内移动。2：2 图片不全部铺满 1 （即缩小时可以有一边出现空隙）。3：2 图片填充整个 1 。'
  )

  test.todo(
    'drag-mode 定义 cropper 的拖拽模式;默认为 "crop";"crop"： 可以产生一个新的裁剪框 3 。"move"：只可以移动图片 3 。"none"： 什么也不处理。'
  )

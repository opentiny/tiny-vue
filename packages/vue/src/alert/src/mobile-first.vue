<template>
  <transition>
    <div v-if="state.show" :class="[
      'min-h-min flex py-[0.435rem] sm:py-3 px-4 my-2 rounded box-border font-light sm:font-normal',
      ...
      <component v-if="showIcon" :is="state.getIcon" custom-class="h-4.5 w-4.5 mt-[0.1875rem] sm:mt-0.5 sm:h-6 sm:w-5 sm:h-5 fill-current" :class="[
      ...
      <div v-if="showFoldable" class="flex-1 leading-6 text-sm overflow-hidden" :class="showIcon ? 'mx-2' : 'mr-2'">
        <div v-if="size === 'large'" @click="handleHeaderClick" class="inline-flex cursor-pointer font-medium">
          ...
          <span class="ml-2.5"><icon-chevron-down class="transition-transform duration-300 align-top my-1 fill-color-icon-placeholder" :class="[state.contentVisible ? 'rotate-180' : 'rotate-0']"></icon-chevron-down></span>
        </div>

        <div ref="ContentDescribe" :class="[
          'transition-all duration-900 ease-linear max-h-[15.75rem]',
          { 'hidden': size === 'large' && !description && !slots.description },
          { 'max-h-0': !state.contentVisible },
        ...
        <div ref="ContentDefault" v-if="size === 'large' && slots.default" :class="[
          'transition-all duration-900 ease-linear max-h-[15.75rem]',
      ...
      <icon-close v-if="!closeText && closable" @click="close" class="h-4 w-4 mt-1 cursor-pointer fill-color-text-primary opacity-70"></icon-close>

<template>
  ...
  <div class="tiny-virtual-scroll" ref="virtualScroll" @scroll="handleScroll" :style="direction === 'vertical'
    ...
    <!-- 调试用，查看direction属性值 -->
    <!-- 内容容器撑起滚动条(使用所有内容高度) -->
    <div class="tiny-virtual-scroll-context"
      ...
      <div class="tiny-virtual-scroll-wrapper" :style="direction === 'vertical'
        ...
        ">
        <!-- {{ JSON.stringify(state.translate) || 'sasas' }} -->
        <!-- 每项内容 -->
        <div class="tiny-virtual-scroll-item" ref="items" :style="direction === 'vertical'
            ? { 'height': `${itemSize}px` || `${estimatedItemSize}` }
            : { 'width': `${itemSize}px ` || `${estimatedItemSize} ` }

<template>
  <div
    ...
    :class="[state.renderType === 'TinyTable' ? 'transferGrid' : '']"
  >
    <p class="tiny-transfer-panel__header">
      ...
      >
        ...
        <div
          class="headSort"
          v-if="
            render && state.renderType !== 'TinyTable' && data.flag === 'sort'
          "
        >
    ...
    <div
      :class="[
        ...
        state.hasFooter ? 'is-with-footer' : ''
      ]"
      ref="reference"
    >
      <tiny-input
      ...
      >
        <template #prefix>
          <i
            :class="['tiny-input__icon', 'tiny-icon-' + state.inputIcon]"
            @click="clearQuery"
          ></i>
        </template>
      </tiny-input>
      ...
      >
        <label
          ...
          :class="[
            ...
            state.checked.indexOf(item[state.keyProp]) > -1
              ? 'is-checked'
              : ''
          ]"
          @click.stop.prevent="
            checkedEvent(item[state.keyProp], item[state.disabledProp])
          "
      ...
      <p
        ...
        v-show="!render && data.length === 0 && !state.hasNoMatch"
      >
    ...
    <p
      ...
      v-if="$parent.slots['left-footer'] || $parent.slots['right-footer']"
    >
      <slot></slot>
    </p>
  </div>
...
import {
  renderless,
  api
} from '@opentiny/vue-renderless/transfer-panel/vue'

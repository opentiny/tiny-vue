...
import { appContext, appProperties, bindFilter, createComponent } from '@opentiny/vue-common/adapter'
import { defineAsyncComponent, directive, emitter, h, markRaw } from '@opentiny/vue-common/adapter'
import { parseVnode, renderComponent, rootConfig, tools, useRouter } from '@opentiny/vue-common/adapter'
...
export const initComponent = () => {
  for (let name in setupComponent) {
    if (Object.prototype.hasOwnProperty.call(setupComponent, name)) {
...
export const $setup = ({ props, context, template }) => {
  const view = hooks.computed(() => {
    if (typeof props.tiny_template !== 'undefined') {
      return props.tiny_template
    }

    const component = template(resolveMode(props, context))
  ...
  return renderComponent({ view, props, context })
}

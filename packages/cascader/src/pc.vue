<template>
  <div
    ref="reference"
    :class="['tiny-cascader', state.realSize && `tiny-cascader--${state.realSize}`, { 'is-disabled': state.isDisabled }]"
  ...
  >
    ...
    <transition name="tiny-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        ...
        ref="popper"
        :class="['tiny-popper', 'tiny-cascader__dropdown', popperClass, { 'is-auto-size': autoSize, 'is-multiple': state.multiple }]"
        @mousedown.stop
      >
        <tiny-cascader-panel
          ref="panel"

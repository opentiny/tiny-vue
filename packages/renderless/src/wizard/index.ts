...
export const nextStepHandle = ({ state, emit }) => () => {
  for (let i = 0; i < state.datas.length; i++) {
...
export const btnSaveHandle = ({ state, emit }) => () => {
  emit('btnSave', state.datas)
}

export const submitHandle = ({ state, emit }) => () => {
...
export const timelineflowData = ({ state, props, api }) => () => {

...
export const handelIconClick = ({ emit }) => (item, index) => {
...
export const getIsGroup = ({ constants, parent: $parent, state }) => () => {
  let parent = $parent
  while (parent) {
...
export const getSize = ({ props, state }) => () => (state.isGroup ? state.columnGroup.size || props.size : props.size)

export const isDisabled = ({ props, state }) => () => props.checkboxDisabled || state.columnGroup.disabled
...
export const setModel = ({ constants, dispatch, emit, state }) => (val) => {
  if (state.isGroup) {

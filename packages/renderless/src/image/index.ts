...
export const computedGetImageStyle = ({ props, api }) => () => {
  const { fit } = props
...
export const computedGetAlignCenter = ({ props, constants }) => () => !isSupportObjectFit() && props.fit !== constants.FILL

export const computedGetPreview = (props) => () => Array.isArray(props.previewSrcList) && props.previewSrcList.length > 0

export const loadImage = ({ state, api, attrs, props }) => () => {
  state.loading = true
  state.error = false
...
export const handleLoad = ({ state, emit }) => (event, img) => {
...
export const handleLazyLoad = ({ state, api, vm, nextTick }) => () => {
...
export const addLazyLoadListener = ({ props, state, api, vm }) => () => {
 ...
 */
export const getImageStyle = ({ state, vm, constants }) => (fit) => {
...
export const mounted = ({ props, api }) => () => {
  if (props.lazy) {

...
export const initPanel = ({ state, api }) => (dateValue, direction, yearNum = 1) => {
...
export const getSelectedPosition = ({ state }) => (dateFormat) => {
...
export const watchVisible = ({ emit, api, nextTick, vm }) => (bool) => {
  if (bool) {
    api.watchModelValue()
...
export const selectOption = ({ emit, state, props }) => ({ yearMonth, index }) => {
  const { type } = props
...
export const selectedComputed = ({ state, props }) => () => {
...
export const scrollLoadHandler = ({ state, vm, api }) => () => {
  const refBody = vm.$refs.datePickerBody

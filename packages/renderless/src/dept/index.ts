...
export const initService = ({ props, service }) => {
  ...
  const noopFnCreator = (propName) => () =>
    Promise.reject(new Error(`[TINY Error][Dept] Prop ${propName} is mandatory when the framework service is not used`))

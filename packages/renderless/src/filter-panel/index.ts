...
export const togglePanel = ({ props, state }) => () => {
  if (props.disabled) return
  state.visible = !state.visible
}

export const popoverShow = ({ state, emit }) => () => {
  emit('visible-change', state.visible)
}

export const popoverHide = ({ state, emit }) => () => {
  emit('visible-change', state.visible)
}

export const handleClear = ({ emit }) => () => {
  emit('handle-clear')
}

/* istanbul ignore next */
export const handleDocumentClick = ({ state, vm }) => (event) => {

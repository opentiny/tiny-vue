...
export const open = ({ api, constants, emit, props, state }) => () => {
  if (state.opened) {
    return
  }
...
export const close = ({ api, constants, emit, props, state }) => () => {
  if (!state.opened) {
    return
  }
...
export const renderOverlay = ({ api, nextTick, props, state }) => () => {
  if (!props.modelValue) {
    return
  }
...
export const updateZIndex = ({ refs, state }) => (value = 0) => (refs.popup.style.zIndex = ++state.context.zIndex + value)

export const clickOverlay = ({ api, props, emit }) => () => {

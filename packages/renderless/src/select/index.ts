...
export const handleQueryChange =
  ({ api, constants, nextTick, props, vm, state }) =>
    (value, isChange = false, isInput = false) => {
      ...
      // 嵌套树时，filterMehod 传递给 tree 组件，然后在上面：vm.$refs.selectTree.filter(value) 强制让 tree 去过滤了。
      // 如果不 return，那么 api.defaultOnQueryChange 内部会再次过滤，而触发错误。
      if (props.renderType === constants.TYPE.Tree) {
        return
      }
  ...
  ({ emit, props, state }) =>
    (event) => {
      ...
      state.willFocusTimer = setTimeout(() => {
        ...
        if (!state.willFocusRun) return // 立即触发了 blur，则不执行 focus 了

        if (!state.softFocus) {
          // tiny 新增 shape 条件：防止过滤器模式，且 filterable 时，面板无法关闭的 bug
...
export const handleBlur =
  ({ constants, dispatch, emit, state, designConfig }) =>
    (event) => {
      ...
      // tiny 新增：表单校验不能异步，否则弹窗中嵌套表单会出现弹窗关闭后再出现校验提示的 bug
      if (!designConfig?.state?.delayBlur) {
        dispatch(constants.COMPONENT_NAME.FormItem, constants.EVENT_NAME.formBlur, event.target.value)
      }
...
export const doDestroy = (vm) => () => {
  // 解决在特殊场景（表格使用 select 编辑器），选中下拉数据的一瞬间 select 组件被销毁时控制台报错的问题
  if (vm?.$refs?.popper) {
  ...
  ({ constants, nextTick, props, vm, state, api, designConfig }) =>
    () => {
      ...
      nextTick(() => {
        ...
        // tiny 新增的 spacing (design 中配置：aui 为 4，smb 为 0，tiny 默认为 0)
...
export const setSoftFocus =
  ({ vm, state, props }) =>
    () => {
      // tiny 新增：解决 reference 插槽时，选择数据后，需要点 2 次才能打开下拉面板
      // 如果有 reference 时，它就没有 Input 这套机制了，没机会让 softFocus 为假了。
      ...
      // tiny 新增：解决获焦即弹出时，关闭不了下拉面板，所以增加了!props.automaticDropdown 条件
...
export const resetInputWidth =
  ({ vm, state }) =>
    () => {
      // tiny 新增：由于当有 reference 插槽时，就没有 vm.$refs.reference 对象了。
...
export const watchOptions =
  ({ api, constants, nextTick, parent, props, state }) =>
    () => {
      ...
      nextTick(() => {
        if (
          parent.$el.querySelector('input') !== document.activeElement && // filterable 时，从 input 框离开了
          !(
            document.activeElement?.classList.contains('tiny-input__inner') && // 并且当前不在下拉面板的 searchable 的 input 中时，才需要更新一下 setSelect
...
export const nodeCheckClick =
  ({ props, state, api }) =>
    (data, { checkedKeys, checkedNodes }) => {
      ...
      // 这行是 aui 的写法，不正确；tiny 是 return checkedNodes, 逻辑也不正确。
...
export const calcCollapseTags =
  ({ state, vm, props }) =>
    () => {
      ...
      const collapseTagWidth = collapseTag && parseFloat(collapseTagContentWidth) + parseFloat(marginRight) // 4 为右 margin 值
...
// tiny 新增：aui 有自己的逻辑，移至 defineConfig 中去了
...
export const computedDisabledTooltipContent =
  ({ state }) =>
    () => {
      // tiny 新增：仅 displayOnly 且传入 options 属性时，不需要渲染 option
      // 禁用的 tooltip 内容 和 仅展示的显示内容，都应该是当前 label 值，共用即可！
 ...
 * aurora 主题 displayOnly||disabled 时显示为 tagText，否则为 tag
 */
...
/**
 * 兼容不同主题多选，tag 在 disabled 和 required 时是否显示关闭按钮的区别
 * default 主题，禁用显示关闭按钮，required 目前和 aurora 保持一致不显示，待设计图补充时更新
...
// 解决无界时，event.target 会变为 wujie_iframe 的元素的 bug

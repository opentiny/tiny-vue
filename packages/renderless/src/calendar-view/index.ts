  ...
  ({ props }) =>
    (day) => {
      ...
      if (isFunction) {
        ...
        // 如果是字符串且包含颜色格式（#开头的十六进制或 rgb），返回样式对象
    ...
    () => {
      ...
      if (props.events && props.events.length) {
        props.events.forEach((prop) => {
          // tiny 新增，这里不能直接改 props.events 里的数据需要进行一次拷贝，不然会导致死循环
          const item = { ...prop }
          const {
            year: startYear,
            month: startMonth,

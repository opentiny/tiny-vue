...
export const initDrag =
  ({ api, emit, markRaw, props, state, vm }) =>
  () => {
    const { config } = props
    const { plugin } = config || {}
    ...
    const handleUpdate = (e) => {
      ...
      if (from === to && oldIndex !== newIndex) {
        ...
        const slct = optDoms.map((optDom) =>
          select.find((opt) => api.keyOption(opt) === api.keyType(optDom.dataset.key))
        )

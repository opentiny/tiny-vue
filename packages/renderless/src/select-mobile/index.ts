...
export const watchModelValue = ({ props, state, emit }) => (value) => {
  ...
  const text = multiple
    ...
    : menus.filter((item) => item[valueField] === value).map((item) => item[textField])
...
export const selectOption = ({ emit, state, props, api }) => (option) => {
...
export const confirm = ({ emit, state, props, api }) => () => {

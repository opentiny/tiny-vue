      ...
      <slot>
        ...
        >
          <div
            ...
            class="
              tiny-mobile-dropdown-item__cell tiny-mobile-dropdown-item__option
            "
            :key="index"
            ...
            <div class="tiny-mobile-dropdown-item__option-title">
              <slot name="title" :titleData="item">
                <span
                  :style="{
                    color:
                      item.value === modelValue && state.activeColor
                        ? state.activeColor
                        : ''
                  }"
                  >{{ item.text }}</span
        ...
        >
          <div class="tiny-mobile-dropdown-item__filter-wrap">
            <div
              ...
              :key="key"
              class="
                tiny-mobile-dropdown-item__cell
                tiny-mobile-dropdown-item__filter-item
              "
            >
              <span class="tiny-mobile-dropdown-item__filter-title">{{
                item.title
              }}</span>
              <ul>
                <li
                  v-for="(tag, tagkey) in options.length === 0
                    ? (item.data = [])
                    : item.data"
                  ...
                  :key="tagkey"
                  :class="[
                    modelValue[key].indexOf(tag.value) > -1 ? 'checked' : ''
                  ]"
                >
                  {{ tag.text }}
                </li>
...
import {
  renderless,
  api
} from '@opentiny/vue-renderless/dropdown-item/vue'

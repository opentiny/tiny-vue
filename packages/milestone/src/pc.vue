<template>
  <div class="tiny-milestone">
    ...
    <div
      ...
      :class="[
        'tiny-milestone__node',
        { 'is-solid': solid },
        `node-status-${node[statusField]}`
      ]"
    >
      <slot
        name="top"
        :slot-scope="node"
        top-slot-class="top-slot-class"
      ></slot>
      <div
        :style="getMileIcon(node)"
        :class="[
          node[statusField] === completedField && !solid ? 'is-completed' : '',
          `status-${node[statusField]}`
        ]"
      ...
      >
        <slot name="icon" :slot-scope="node">
          <icon-yes
            v-if="node[statusField] === completedField"
            class="tiny-svg-size"
          />
      ...
      <div
        :class="[
          'tiny-milestone__line',
          {
            ...
            'tiny-milestone__line-completed-custom':
              node[statusField] === 'completed'
          }
        ]"
        ...
        <div v-if="index < data.length - 1">
          <div
            ...
            :style="{
              left: `calc(${
                (100 /
                  (data[flagBefore ? index : index + 1][flagField].length +
                    1)) *
                (idx + 1)
              }% - 29px)`
            }"
            :key="idx"
            :class="[
              'tiny-milestone__flag',
              `flag-status-${flag[flagStatusField]}`
            ]"
          >
            <div

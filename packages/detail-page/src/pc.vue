  ...
  <div class="tiny-detail-page">
    ...
    <ul class="tiny-detail-page__content">
      <li
        ...
        v-show="!item.hidden"
      >
        <span class="tiny-detail-page__content-item-span"
          >{{ item.label }}{{ state.textSplit }}</span
        >{{ item.value }}
      </li>
    </ul>
    <tiny-modal
      ...
      <div class="tiny-detail-dialog">
        <div class="tiny-detail-dialog__header">
          ...
          <div class="tiny-detail-dialog__header-check">
            <tiny-checkbox
              ...
              @change="checkAllHander"
              :indeterminate="state.indeterminate"
              >{{ state.valueTitle }}</tiny-checkbox
            >
          </div>
        </div>
        <ul class="tiny-detail-dialog__content">
          <li
            ...
            :key="index"
            :class="[
              'tiny-detail-dialog__content-item',
              { active: item.hidden }
            ]"
          >
            ...
            <tiny-checkbox
              v-model="item.hidden"
              @click.prevent="checkValueHiddenItem"
              style="margin-right: 113px"
            ></tiny-checkbox>
          </li>
        </ul>
        <div class="tiny-detail-dialog__footer">
          <tiny-button type="primary" @click="saveHandler">{{
            state.saveButton
          }}</tiny-button>
...
import {
  renderless,
  api
} from '@opentiny/vue-renderless/detail-page/vue'

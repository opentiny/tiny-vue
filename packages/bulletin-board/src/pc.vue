    ...
    <tabs v-model="state.actName">
      ...
      >
        ...
        <div class="tiny-bulletin-board__contain">
          <div
            v-for="(subItem, subIndex) in state.dataList"
            :key="subIndex"
          ...
          >
            ...
            >
              <span v-if="subIndex === 0" class="tiny-bulletin-board__new">
                <component :is="icon" class="tiny-svg-size" />{{
                  icon ? '' : 'NEW'
                }}
              </span>
              {{ subItem.text }}
          ...
          <div v-if="showMore" class="tiny-bulletin-board__more">
            <a
              v-if="
                showMore &&
                state.moreLink &&
                (state.moreLink.url || state.moreLink.route)
              "
...
import {
  renderless,
  api
} from '@opentiny/vue-renderless/bulletin-board/vue'

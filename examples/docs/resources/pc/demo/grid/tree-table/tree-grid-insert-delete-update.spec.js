...
test.describe('树表增删改功能', () => {
  test('树表增删测试', async ({ page }) => {
    ...
    await page.getByRole('row', { name: '1 新数据 ee' }).locator('i').click()
    await page.getByText('新数据', { exact: true }).click()
  ...
  test('树表修改测试', async ({ page }) => {
    ...
    await page.getByRole('row', { name: '3 WWWW科技股份有限子公司 华南区 720' }).getByText('华南区').click()
    await page.getByRole('row', { name: '3 WWWW科技股份有限子公司 720' }).getByRole('textbox').click()

...
test('已完成节点实心显示', async ({ page }) => {
  ...
  const nodeCount = 6
  const iconClasss = [/is-completed status-completed/, /is-completed status-completed/, /status-doing/, /status-xxx/, /status-back/, /status-end/]
  const iconStyles = [
  ...
  const titles = ['完成状态，状态值:completed', '完成状态，状态值:completed', '未完成状态, 状态值:doing', '未完成状态，状态值:xxx', '未完成状态，状态值:back', '未完成状态，状态值:end']

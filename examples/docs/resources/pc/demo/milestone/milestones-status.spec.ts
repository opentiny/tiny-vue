...
test('状态与颜色对应关系', async ({ page }) => {
  page.on('pageerror', (exception) => expect(exception).not.toBeNull())
  await page.goto('http://127.0.0.1:7130/pc/milestone/milestones-status')
  ...
  const titles = ['完成状态，状态值:completed', '完成状态，状态值:completed', '未完成状态, 状态值:doing', '未完成状态，状态值:cancel', '未完成状态，状态值:back', '未完成状态，状态值:end']

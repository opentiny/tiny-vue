<template>
  <tiny-grid :data="tableData" :edit-config="{ trigger: 'click', mode: 'cell', showStatus: true }">
    ...
    <tiny-grid-column
      ...
      :editor="{
        component: Popeditor,
        attrs: getPopEditorOp,
        events: popEditorEvents
      }"
      :format-config="{
        type: 'enum',
        async: true,
...
import { Grid as TinyGrid, GridColumn as TinyGridColumn, Popeditor, Modal } from '@opentiny/vue'
...
function pageChange(grid, page) {
  Modal.message({ message: `当前第 ${page} 页`, status: 'info' })
}

function valueChange({ row }) {
  Modal.message({ message: `当前选中 ${row.name}`, status: 'info' })
}

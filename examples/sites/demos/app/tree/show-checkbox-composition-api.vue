<template>
  <div>
    ...
    >
    <tiny-button @click="setCheckedNodes" title="通过 node 设置节点的当前选中状态"
      >通过 node 设置节点的当前选中状态</tiny-button
    >
    <tiny-button @click="updateKeyChildren" title="通过 keys 设置节点子元素">通过 keys 设置节点子元素</tiny-button>
    <tiny-tree :data="data" ref="treeRef" default-expand-all show-checkbox node-key="id"></tiny-tree>
  </div>
</template>
...
function getNode() {
  Modal.message({
    message: `${treeRef.value.getNode(3).data.label}`,
    status: 'info'
  })
}

function setChecked() {
  treeRef.value.setChecked(treeRef.value.getNode(8).data, true)
}

function setCheckedKeys() {
  treeRef.value.setCheckedKeys([9])
}

function setCheckedNodes() {
  treeRef.value.setCheckedNodes([{ id: 10, label: '三级 1-1-2' }])
}

function updateKeyChildren() {
  treeRef.value.updateKeyChildren(8, [{ id: 11, label: '三级 3-3-1' }])
}
</script>

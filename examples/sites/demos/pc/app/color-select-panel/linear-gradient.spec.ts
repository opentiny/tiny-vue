...
test('线性渐变 (历史记录)', async ({ page }) => {
  const demo = page.locator('#linear-gradient')
  const colorInput = demo.locator('.tiny-color-select-panel__tools-hex1 input')
  await page.goto('color-select-panel#linear-gradient')
  ...
  await expect(colorInput).toHaveValue('linear-gradient(120deg, hsla(334, 80%, 40%, 0.32) 0%,#F48FA2FF 96%)')
  await page.locator('.tiny-color-select-panel__history__color-block').click()
  await expect(colorInput).toHaveValue('linear-gradient(120deg, hsla(201, 80%, 40%, 0.32) 0%,#8FF2F4FF 96%)')
...
test('线性渐变 (预定义颜色)', async ({ page }) => {
  ...
  await expect(colorInput).toHaveValue('linear-gradient(180deg, #FFFFFFFF 0%,#66CCFFFF 100%)')
  await page.getByRole('button', { name: '确定' }).click()
  await demo.getByRole('button', { name: 'Show Color select panel' }).click()
  await page.locator('.tiny-color-select-panel__history__color-block').first().click()
  await expect(colorInput).toHaveValue('linear-gradient(120deg, hsla(201, 80%, 40%, 0.32) 0%,#8FF2F4FF 96%)')

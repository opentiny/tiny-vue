...
test('多级菜单，使用 menu-options', async ({ page }) => {
  ...
  await page.waitForTimeout(500)
  await dropDownMenuItem.getByText('老友粉 2.1').hover()
  await expect(dropDownMenuItem.getByText('狮子头 3.1')).toBeVisible()
  await dropDownMenuItem.getByText('狮子头 3.1').click()
  await page.waitForTimeout(1000)

  await expect(
    page.locator('.tiny-modal').filter({
      hasText: '配置式可以通过 data.itemData 获取配置数据：{"label":"狮子头 3.1"}'
    })
  ).toHaveCount(1)
  await page.waitForTimeout(500)
})

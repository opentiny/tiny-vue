...
test('基本用法', async ({ page }) => {
  ...
  await page.goto('virtual-scroll#horizontal-scroll')
  const virtualScroll = page.locator('#horizontal-scroll .tiny-virtual-scroll')
  ...
  await expect(visibleCount).toBe(12)
  for (let i = 0; i < visibleItems.length; i++) {
    console.log(`Item ${i + 1}:`, visibleItems[i])
  }
  await virtualScroll.evaluate((scroll) => scroll.scrollTo(50000, 0))
  await page.waitForTimeout(500)
  const targetItem = virtualScroll.locator('.tiny-virtual-scroll-item', { hasText: '第500项' })
  await expect(targetItem).toBeVisible()
  await virtualScroll.evaluate((scroll) => scroll.scrollTo(100000, 0))

...
test('表格过滤', async ({ page }) => {
  ...
  await page
    ...
    .click()
  await expect(page.getByRole('cell', { name: 'RFV 有限责任公司' })).toHaveCount(0)
  await expect(page.getByRole('cell', { name: 'WWW 科技 YX 公司' })).toHaveCount(1)

  // 清楚筛选
  await page.getByRole('button', { name: '清空表格 name 字段的过滤条件' }).click()
  await expect(page.getByRole('cell', { name: 'RFV 有限责任公司' })).toHaveCount(1)
})

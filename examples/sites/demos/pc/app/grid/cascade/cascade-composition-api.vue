  ...
  <div>
    ...
    >
      ...
      <tiny-grid-column
        field="city"
        title="城市(级联三级)"
        :editor="{ component: CustomSelect, attrs: attrs3 }"
      ></tiny-grid-column>
    </tiny-grid>
  </div>
</template>
...
function changeArea({ $table, row }, value) {
  row.area = value
  row.province = null
  row.city = null
...
function changeProvince({ $table, row }, value) {
  row.province = value
  row.city = null
  $table.setActiveCell(row, 'city')
}

function activeMethod({ column, row }) {
  if (column.property === 'province') {
    setTimeout(() => {
      attrs2.value.data = getCascadeData({
        type: 'province',
        area: row.area
      })
    }, 1000)
    ...
    setTimeout(() => {
      attrs3.value.data = getCascadeData({
        type: 'city',

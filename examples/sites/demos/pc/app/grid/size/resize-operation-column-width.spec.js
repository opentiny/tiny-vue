...
test('操作列开启列宽拖拽', async ({ page }) => {
  page.on('pageerror', (exception) => expect(exception).toBeNull())
  await page.goto('grid-size#size-resize-operation-column-width')
  const draggerDom = page.locator('th.col__radio').locator('.tiny-grid-resizable')
  const thDom = page.locator('th.col__radio')
  // 获取初始列宽

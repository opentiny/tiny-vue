...
test('清空数据', async ({ page }) => {
  ...
  await page.goto('grid-toolbar#toolbar-clear-data')
  await page.getByRole('button', { name: '清空表格第一行数据' }).click()
  await expect(page.getByText('GFD 科技 YX 公司')).toHaveCount(0)
  await page.getByRole('button', { name: '清空公司名称字段数据' }).click()
  await expect(page.getByText('深圳市福德宝网络技术 YX 公司')).toHaveCount(0)

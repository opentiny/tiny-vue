export default {
  ...
  demos: [
    ...
    },
    {
      ...
      desc: {
        'zh-CN':
          '配置 <code> pre </code>  为  <code> true </code> ，就会预格式化  <code> content </code>  文本。<br>\n        被包围在 &lt;pre&gt; &lt;/pre&gt 标签元素中的文本会保留空格和换行符，文本也会呈现为等宽字体。',
        'en-US':
          'If <code> pre </code> is set to <code> true </code>, the <code> content </code> text is preformatted.<br>\n          Text enclosed in the <pre> </pre&gt tag element retains spaces and newline characters, and the text is rendered in a uniform font.'
      },
      codeFiles: ['pre.vue']
    }
  ]

...
test('测试按钮是否禁用', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Button1' }).nth(2)).not.toHaveClass(/disabled/)
  await expect(page.getByRole('button', { name: 'Button2' }).nth(2)).toHaveClass(/disabled/)
})

...
test('时间选择器基本用法', async ({ page }) => {
  ...
  // 测试滚动选择时间
  const timePicker = page.getByRole('textbox', { name: '18:42:20' }).first()
  await page.getByRole('textbox', { name: '18:40:20' }).first().click()
  await page.getByRole('listitem').filter({ hasText: '42' }).first().click()
  await page.getByRole('button', { name: '确定' }).click()
  await expect(timePicker).toBeVisible()

  // 测试箭头选择时间
  const timePickerArrowControl = page.getByRole('textbox', { name: '19:42:20' }).nth(1)
  await page.getByRole('textbox', { name: '18:42:20' }).nth(1).click()

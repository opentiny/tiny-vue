export default {
  ...
  demos: [
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p>通过 <code>pop-position</code> 属性设置箭头方向。<br>该属性的可选值为 <code>top/top-start/top-end/bottom/bottom-start/bottom-end/left/left-start/left-end/right/right-start/right-end</code> <br>若存在多个步骤，需要单独给每个步骤设置不同的展示位置，可以在 <code>dom-data</code> 里面添加 <code>popPosition</code> 属性并赋值，若存在单独设置的箭头方向，则会覆盖全局的箭头方向</p>',
        'en-US': ''
      },
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p>窗口的事件回调<br>在 <code>dom-data<code> 中使用：<br> <code>beforeShow</code> ：展示前回调<br> <code>show</code> ：展示回调<br> <code>beforeHide</code> ：隐藏前回调<br> <code>hide</code> ：隐藏回调<br> <code>cancel</code> ：关闭取消回调<br> <code>complete</code> ：完成回调<br>  <code>destroy</code> ：销毁回调</p>',
        'en-US': ''
      },
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p>通过设置 <code>main-axis</code> 大小来设置主轴（纵轴）距离</p>',
        'en-US': ''
      },
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p>通过设置 <code>cross-axis</code> 大小来设置横轴距离</p>',
        'en-US': ''
      },
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p>通过设置 <code>alignment-axis</code> 大小来设置对齐轴距离</p>',
        'en-US': ''
      },
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p><code>modal-overlay-opening-padding</code> ：可以在模态叠加层开口周围添加的边界半径量，控制引导元素高亮范围<br><p><code>modal-overlay-opening-radius</code> ：可以在模态叠加层开口周围添加的填充量，控制引导元素高亮圆角</p>',
        'en-US': ''
      },

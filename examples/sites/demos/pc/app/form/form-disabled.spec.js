...
test('测试表单禁用', async ({ page }) => {
  ...
  const slider = formItem.nth(19).locator('.tiny-slider__handle')
  ...
  await expect(formItem.nth(10).locator('.tiny-numeric__input')).toHaveClass(/is-disabled/)
  await expect(formItem.nth(11).locator('input').first()).toBeDisabled()
  await expect(formItem.nth(12).locator('input').first()).toBeDisabled()
  ...
  await expect(formItem.nth(10).locator('.tiny-numeric__input')).not.toHaveClass(/is-disabled/)
  await expect(formItem.nth(11).locator('input').first()).not.toBeDisabled()
  await expect(formItem.nth(12).locator('input').first()).not.toBeDisabled()

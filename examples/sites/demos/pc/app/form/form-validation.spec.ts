...
test('测试表单校验规则', async ({ page }) => {
  ...
  await expect(page.getByRole('alert', { name: '必填' }).first()).toBeVisible()
  await expect(page.getByRole('alert', { name: '不符合规则的日期格式' })).toBeVisible()
  await expect(page.getByRole('alert', { name: '必填' }).nth(1)).toBeVisible()
  await expect(page.getByRole('alert', { name: '必填' }).nth(2)).toBeVisible()
  await expect(page.getByRole('alert', { name: '必填 2~11 之间的数字' })).toBeVisible()
  await expect(page.getByRole('alert', { name: '必填 1.1.1.1' })).toBeVisible()
})
...
test('测试表单输入变化和失焦是否出现校验', async ({ page }) => {
  ...
  // 更新为 role='alert'，因为表单验证错误提示现在使用 alert 角色以提供更好的可访问性
  const requiredTip = page.getByRole('alert', { name: '必填' })
  ...
  await expect(page.getByRole('alert', { name: '长度必须不小于 2' })).toBeVisible()
  await firstInput.fill('111')
  await expect(page.getByRole('alert', { name: '长度必须不小于 2' })).not.toBeVisible()

  // 日期选择器的校验
  const datePicker = form.locator('.tiny-date-editor').first().locator('input')
  await datePicker.click()
  // 点击第一项输入去触发失焦，调用 blur 事件不会将日期选择器收起
  await firstInput.click()
  await expect(page.getByRole('alert', { name: '不符合规则的日期格式' })).toBeVisible()
  await datePicker.click()
  await page.locator('.tiny-date-picker .tiny-date-table__row').getByText('15').first().click()
  await expect(page.getByRole('alert', { name: '不符合规则的日期格式' })).not.toBeVisible()

  // url 输入框
  const urlInput = formItem.nth(4).locator('input')
  const validUrl = 'https://test.com'
  urlInput.fill('1111')
  await expect(page.getByRole('alert', { name: '非法 URL 地址' })).toBeVisible()
  urlInput.fill(validUrl)
  await expect(page.getByRole('alert', { name: '非法 URL 地址' })).not.toBeVisible()

  // email 输入框
  const emailInput = formItem.nth(5).locator('input')
  const validEmail = '111@test.com'
  emailInput.fill('1111')
  await expect(page.getByRole('alert', { name: '非法邮件地址' })).toBeVisible()
  emailInput.fill(validEmail)
  await expect(page.getByRole('alert', { name: '非法邮件地址' })).not.toBeVisible()
  ...
  await expect(page.getByRole('alert', { name: '必填 2~11 之间的数字' })).toBeVisible()
  await increase.click()
  await expect(page.getByRole('alert', { name: '必填 2~11 之间的数字' })).not.toBeVisible()
  ...
  await expect(page.getByRole('alert', { name: '必填 1.1.1.1' })).not.toBeVisible()

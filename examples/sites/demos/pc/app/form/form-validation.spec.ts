...
test('测试表单校验规则', async ({ page }) => {
  ...
  await expect(page.getByRole('tooltip', { name: '必填 1.1.1.1' })).toBeVisible()
})
...
test('测试表单输入变化和失焦是否出现校验', async ({ page }) => {
  ...
  await expect(page.getByRole('tooltip', { name: '长度必须不小于 2' })).toBeVisible()
  await firstInput.fill('111')
  await expect(page.getByRole('tooltip', { name: '长度必须不小于 2' })).not.toBeVisible()
  ...
  await expect(page.getByRole('tooltip', { name: '必填 1.1.1.1' })).not.toBeVisible()

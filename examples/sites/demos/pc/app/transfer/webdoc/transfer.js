  ...
  demos: [
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': `
          ...
          通过 <code> default </code> 插槽,自定义数据项渲染的dom结构，推荐使用插槽的方式。<br>
          通过 <code> render-content </code> 属性函数,使用JSX功能，去自定义数据项渲染的dom结构。<br>
...

    {
      ...
      'desc': {
        ...
        'en-US': `<div class="tip custom-block"><p class="custom-block-title"> Please install sortablejs plugin before using drag and drop function </div> Pass The <code>drop-config</code> property config the sortablejs plugin to do the left-right drag shuttle.`
      },
    ...
    },
    {
      ...
      'desc': {
        'zh-CN':
          '当 <code>render</code> 属性里的 <code>plugin</code> 设置为 <code>Table</code> 时指定穿梭框渲染成表格，具体配置可参考 <code>grid</code> 组件的配置。\n',
        'en-US':
          'The <code>plugin</code> in the <code>render</code> attribute is set to <code>Table</code>, which is specified in the <code>grid</code> component configuration.\n'
      },

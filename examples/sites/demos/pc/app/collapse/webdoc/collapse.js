  ...
  demos: [
    {
      ...
      'desc': {
        'zh-CN': '<p>详细用法参考如下示例 <br/>配置 <code>name</code> 属性作为每个 collapse-item 的唯一标志符。</p>',
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p>配置 <code>accordion</code> 属性为 true 后，折叠面板将展示手风琴效果 <br/> 配置 <code>v-model</code> 设置当前激活的面板(如果是手风琴模式，绑定值类型需要为 string ，否则为 array )</p>',
    ...
    },
    {
      ...
      'desc': {
        'zh-CN': '<p>在 <code>collapse-item</code> 元素上配置 <code>title</code> 属性可以指定每个折叠面板项的标题。但同时也可以通过 <code>title</code> 插槽的方式自定义面板标题，比如在标题前增加图标。<br/>在 <code>collapse-item</code> 元素上配置 <code>title-right</code> 属性可以指定每个折叠面板项标题的右侧内容。但同时也可以通过 <code>title-right</code> 插槽的方式自定义面板标题右侧内容，比如在标题右侧增加图标 </p>',

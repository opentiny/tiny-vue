export default {
  ...
  demos: [
    ...
    },
    {
      ...
      desc: {
        'zh-CN':
          '<p>通过添加`component-empty`属性展示组件级异常，其中 type 类型有`noperm`、 `nodata`、 `weaknet`、 `noresult`、 `nonews`<br> 对应场景：<br>`noperm` ：无访问权限<br>`nodata` ：暂无数据<br>`weaknet` ：网络不给力<br>`noresult`：无相关搜索结果<br>`nonews`：暂无最新消息<br></p>',
        'en-US':

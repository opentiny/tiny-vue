<template>
  <div>
    <div class="demo-query-builder--controler">
      <label v-for="key in Object.keys(optionOrder)" :key="key">
        <input
          type="checkbox"
          :checked="optionOrder[key]"
          @change="
            (e) =>
              updateOptions({
                action: 'update',
      ...
      <div style="border-top: 1px solid #eee; padding-top: 10px">
        <span v-for="(item, key) in actions" :key="key">
          <tiny-button type="primary" plain @click="updateOptions({ action: item.action })">{{
            item.text
          }}</tiny-button>
        </span>
      </div>
...
<script>
import { QueryBuilder, Button } from '@opentiny/vue'
...
export default {
  ...
  methods: {
    getFormateData() {
      this.formatData = formatMap.map(([format]) => {
    ...
    updateOptions({ action, payload }) {
      if (action === 'reset') {
        this.optionOrder = { ...defaultOptions }
        this.updateConfig()
      } else if (action === 'all') {
        const allSelected = { ...defaultOptions }
        for (const opt of Object.keys(this.optionOrder)) {
          allSelected[opt] = ![
            'disabled',
        ...
        this.optionOrder = allSelected
        this.updateConfig()
      } else if (action === 'update') {
        const { optionName, value } = payload
        this.optionOrder = { ...this.optionOrder, [optionName]: value }
        this.updateConfig()
      } else if (action === 'replace') {
        this.query = action.payload
      } else if (action === 'clearQuery') {
        this.query = {
          'combinator': 'and',
          'rules': [
            {
      ...
      } else if (action === 'resetQuery') {
        this.query = this.getInitialQuery()
      }
    },
    updateConfig() {
      this.config = { ...this.config, ...this.optionOrder }
    },
    getInitialQuery() {
      const initialQuery = {
        id: generateID(),
...
<style scoped>
.demo-query-builder--controler {
  padding: 10px;
  border: 1px solid #eee;
  margin-bottom: 20px;
}

.demo-query-builder--controler label {
  padding: 2px 10px;
  font-size: 14px;
...
.demo-query-builder--controler span {
  padding: 5px 10px;
  display: inline-block;
  min-width: 20%;
}
